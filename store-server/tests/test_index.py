...
def test_parse_picks_latest_version_and_absolute_urls():
    ...
    todo = next(a for a in apps if a["packageName"] == "dev.openap.apps.todo")
    assert todo["name"] == "Todo"
    assert todo["summary"] == "Simple todo list"
    assert todo["versionCode"] == 2
    assert todo["versionName"] == "0.2.0"
    assert todo["apkUrl"] == "http://store.test/fdroid/repo/dev.openap.apps.todo_2.apk"
    assert todo["apkName"] == "dev.openap.apps.todo_2.apk"
    assert todo["iconUrl"] == "http://store.test/fdroid/repo/icons-640/dev.openap.apps.todo.2.png"
...
def test_parse_falls_back_to_any_locale_and_sorts_by_last_updated():
    apps = parse_index_v2(FIXTURE, "http://store.test/fdroid/repo")
    assert [a["packageName"] for a in apps] == ["dev.openap.apps.todo", "dev.openap.apps.old"]
    old = apps[1]
...
def test_find_package():
    assert find_package(FIXTURE, "dev.openap.apps.todo", 1)["manifest"]["versionName"] == "0.1.0"
    assert find_package(FIXTURE, "dev.openap.apps.todo", 9) is None
